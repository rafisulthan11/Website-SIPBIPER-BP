        ...
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <div class="p-4 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                        ...
                        <div class="bg-gradient-to-r from-purple-50 to-purple-100 border-l-4 border-purple-600 rounded-lg p-4 shadow-md">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Pembudidaya Terdaftar</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-blue-100 border-l-4 border-blue-600 rounded-lg p-4 shadow-md">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Dari {{ $uniqueRTP ?? 0 }} Pembudidaya</p>
                                </div>
                            </div>
                        </div>

                        <!-- Total Luas Kolam Card -->
                        <div class="bg-gradient-to-r from-green-50 to-green-100 border-l-4 border-green-600 rounded-lg p-4 shadow-md">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 class="text-sm font-medium text-gray-600">Total Luas Kolam</h4>
                                    <p class="text-2xl font-bold text-green-700">{{ number_format($totalLuasKolam, 0, ',', '.') }} m²</p>
                                    <p class="text-xs text-gray-500 mt-1">Dari {{ $uniqueRTP ?? 0 }} Pembudidaya</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Table controls -->
                    <form method="GET" action="{{ route('laporan.rekapitulasi.pembudidaya') }}" id="searchForm">
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 mb-3"
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center gap-2"
                                ...
                                <a href="{{ route('laporan.rekapitulasi.pembudidaya.export', request()->query()) }}" title="Unduh Excel — semua hasil filter" aria-label="Unduh Excel untuk semua hasil filter" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm w-full sm:w-auto"
                    ...
                    <div class="md:hidden space-y-3 mb-4">
                        ...
                        <div class="rounded-lg border border-slate-200 p-4 bg-white shadow-sm">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <p class="text-xs text-slate-600">{{ $p->jenis_kegiatan_usaha ?? '-' }}</p>
                            </div>
                            <div class="mt-2 text-sm text-slate-700 space-y-1">
                                <p><span class="font-medium">Komoditas:</span> @if($p->ikan->count() > 0){{ $p->ikan->pluck('jenis_ikan')->unique()->implode(', ') }}@else-@endif</p>
                                <p><span class="font-medium">Total Luas Kolam:</span> {{ number_format($p->total_luas_kolam_pembudidaya ?? 0, 0, ',', '.') }} m²</p>
                                <p><span class="font-medium">Total Produksi:</span> {{ number_format($p->total_produksi_pembudidaya ?? 0, ($p->satuan_produksi == 'Ton' ? 2 : 0), ',', '.') }} {{ $p->satuan_produksi ?? 'Kg' }}</p>
                            </div>
                            <div class="mt-3 flex flex-wrap gap-2">
                                <a href="{{ route('pembudidaya.show', ['pembudidaya' => $p->id_pembudidaya, 'from_report' => 1]) }}" class="inline-flex items-center rounded bg-green-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-green-700">Lihat Detail</a>
                                <a href="{{ route('laporan.rekapitulasi.pembudidaya.pdf', $p->id_pembudidaya) }}" class="inline-flex items-center rounded bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">Unduh PDF</a>
                            </div>
                        </div>
                        @empty
                            ...
                            <table class="min-w-full text-base">
                                <thead class="bg-slate-100 text-slate-800">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-left font-semibold text-[15px]">Jenis Kegiatan Usaha</th>
                                        <th class="px-4 py-3 text-right font-semibold text-[15px]">Total Luas Kolam (m²)</th>
                                        <th class="px-4 py-3 text-right font-semibold text-[15px]">Total Produksi</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-t border-slate-200">
                                        ...
                                        <td class="px-4 py-3 align-top text-slate-700">
                                            @if($p->ikan->count() > 0)
                                                {{ $p->ikan->pluck('jenis_ikan')->unique()->implode(', ') }}
                                            @else
                                                -
                                            @endif
                                        </td>
                                        <td class="px-4 py-3 align-top text-slate-700">{{ $p->jenis_kegiatan_usaha ?? '-' }}</td>
                                        <td class="px-4 py-3 align-top text-right text-slate-700">{{ number_format($p->total_luas_kolam_pembudidaya ?? 0, 0, ',', '.') }} m²</td>
                                        <td class="px-4 py-3 align-top text-right text-slate-700">
                                            {{ number_format($p->total_produksi_pembudidaya ?? 0, ($p->satuan_produksi == 'Ton' ? 2 : 0), ',', '.') }} {{ $p->satuan_produksi ?? 'Kg' }}
                                        </td>
                                        <td class="px-4 py-3 align-top">
                                            <div class="flex flex-wrap gap-2">
                                                <a href="{{ route('pembudidaya.show', ['pembudidaya' => $p->id_pembudidaya, 'from_report' => 1]) }}" class="inline-flex items-center rounded bg-green-600 px-3.5 py-1.5 text-sm font-semibold text-white hover:bg-green-700">
