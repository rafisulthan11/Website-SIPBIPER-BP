<x-app-layout>
            ...
            <div x-data="{ step: {{ $initialStep }}, maxStep: 1 }" class="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
                <form method="POST" action="{{ route('harga-ikan-segar.store') }}">
                    ...
                    <div class="px-6 pt-4 pb-2">
                        <h3 class="text-lg font-semibold text-slate-800">Tambah Data Harga</h3>
                    </div>
                        ...
                        <div x-show="step===0" x-transition class="bg-gray-50 rounded-lg border border-gray-200 p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="tanggal_input" class="block mt-1 w-full" type="date" name="tanggal_input" :value="old('tanggal_input', date('Y-m-d'))" required />
                                ...
                                <div>
                                    ...
                                    <select name="nama_pasar" id="nama_pasar" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm bg-gray-100" required disabled>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="asal_ikan" class="block mt-1 w-full" type="text" name="asal_ikan" :value="old('asal_ikan')" placeholder="Contoh: Tambak Wono, Surabaya" />
                                ...
                                <div class="md:col-span-2 lg:col-span-3">
                                    ...
                                    <textarea id="keterangan" name="keterangan" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" rows="3" placeholder="Catatan tambahan tentang kondisi pasar atau informasi harga">{{ old('keterangan') }}</textarea>
                            ...
                            <template x-for="(ikan, index) in ikanList" :key="index">
                                <div class="mb-6 p-4 bg-white rounded-lg border border-gray-200">
                                    <div class="flex justify-between items-center mb-4">
                                        ...
                                        <button type="button" @click="ikanList.splice(index, 1)" x-show="ikanList.length > 1" class="px-3 py-1.5 text-sm text-red-600 hover:text-white hover:bg-red-600 border border-red-600 rounded transition">Hapus</button>
                                    </div>
    ...
    <script>
        ...
        const oldPasarValue = '{{ old("nama_pasar") }}';

        // Kecamatan -> Desa
        kecamatanSelect.addEventListener('change', function() {
            const idKecamatan = this.value;
            
            // Reset desa dan pasar
            ...
            if (idKecamatan) {
                fetch(`/api/desa-by-kecamatan/${idKecamatan}`)
                    ...
                    .then(data => {
                        ...
                        // Trigger desa change if old value exists
                        if (oldDesaValue) {
                    ...
                    .then(data => {
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(pasar => {
                                ...
                                if (oldPasarValue && oldPasarValue == pasar.nama_pasar) {
                                    option.selected = true;
                                }
        ...
        // Trigger change if old kecamatan exists
