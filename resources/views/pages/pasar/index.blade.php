        ...
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="rounded-lg overflow-hidden shadow-md">
                ...
                <div class="bg-blue-600 text-white px-6 py-4">
                    <h3 class="text-2xl font-bold">Kelola Data Pasar</h3>
                </div>
                ...
                <div class="bg-white border-x border-b border-slate-200">
                    ...
                    <div class="p-5">
                        <!-- Title -->
                        <div class="mb-4">
                            <h4 class="text-slate-800 font-semibold text-lg">Daftar Pasar</h4>
                        </div>
                    
                    <!-- Show entries, Search and Add Button -->
                    <form method="GET" action="{{ route('pasar.index') }}" class="space-y-3">
                        ...
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3"
                            ...
                            <a href="{{ route('pasar.create') }}" class="inline-flex items-center justify-center bg-blue-700 hover:bg-blue-800 text-white font-medium text-sm rounded-lg px-4 py-1.5 shadow whitespace-nowrap">
                ...
                <div class="px-5 pb-5">
                    ...
                    <div class="md:hidden space-y-3">
                        @forelse ($pasar as $p)
                            <div class="rounded-lg border border-slate-200 p-4 bg-white shadow-sm">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        <p class="font-semibold text-slate-800">{{ $p->nama_pasar ?? '-' }}</p>
                                        <p class="text-sm text-slate-600">{{ $p->desa ?? '-' }}, {{ $p->kecamatan ?? '-' }}</p>
                                    </div>
                                    <div>
                                        @if($p->status == 'aktif')
                                ...
                                <div class="mt-3 flex flex-wrap gap-2">
                                    <a href="{{ route('pasar.show', $p->id_pasar) }}" class="inline-flex items-center rounded bg-green-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-green-700">Lihat</a>
                                    <a href="{{ route('pasar.edit', $p->id_pasar) }}" class="inline-flex items-center rounded bg-yellow-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-yellow-600">Edit</a>
                                    <form action="{{ route('pasar.destroy', $p->id_pasar) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <div class="rounded-lg border border-slate-200 p-4 text-center text-slate-500">Belum ada data pasar.</div>
                        @endforelse
                    </div>
                        ...
                        <table class="min-w-full text-base">
                            ...
                            <tbody>
                                ...
                                <tr class="border-t border-slate-200">
                                    <td class="px-4 py-3 align-top text-slate-700">{{ $p->nama_pasar ?? '-' }}</td>
                                    ...
                                    <td class="px-4 py-3 align-top">
                                        <div class="flex flex-wrap gap-2">
                                            <a href="{{ route('pasar.show', $p->id_pasar) }}" class="inline-flex items-center rounded bg-green-600 px-3.5 py-1.5 text-sm font-semibold text-white hover:bg-green-700">
                                                Lihat
                                            </a>
                                            <a href="{{ route('pasar.edit', $p->id_pasar) }}" class="inline-flex items-center rounded bg-yellow-500 px-3.5 py-1.5 text-sm font-semibold text-white hover:bg-yellow-600">
                                                Edit
                                            </a>
                                            <form action="{{ route('pasar.destroy', $p->id_pasar) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');" class="inline">
                                                @csrf
                    ...
                    <!-- Pagination -->
                    <div class="mt-4">
                        {{ $pasar->onEachSide(1)->links('components.pagination.custom') }}
                    </div>
                </div>
            </div>
            </div>
