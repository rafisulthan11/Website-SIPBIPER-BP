        ...
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="rounded-lg overflow-hidden shadow-md">
                ...
                <div class="p-5">
                    ...
                    <form method="GET" action="{{ route('pembudidaya.index') }}" class="space-y-3">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3" style="row-gap: 0.75rem; column-gap: 1rem;">
                            ...
                            <div class="w-full md:w-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:ml-auto" style="gap: 0.75rem;">
                                ...
                                @if(auth()->user()->role->nama_role === 'staff')
                                    <a href="{{ route('pembudidaya.create') }}" class="inline-flex items-center justify-center bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 text-white font-semibold text-sm rounded-lg px-5 py-2 shadow-md hover:shadow-lg transition-all duration-200 whitespace-nowrap">
                ...
                <div class="px-5 pb-5">
                    ...
                    <div class="md:hidden space-y-3">
                        @forelse ($pembudidayas as $p)
                            <div class="rounded-lg border border-slate-200 p-4 bg-white shadow-sm">
                                ...
                                <div class="text-sm text-slate-700 space-y-1">
                                    ...
                                    <p><span class="font-medium">Komoditas:</span>
                                        @if($p->kolam->count() > 0)
                                ...
                                <div class="mt-3 flex flex-wrap gap-2">
                                    <a href="{{ route('pembudidaya.show', $p->id_pembudidaya) }}" class="inline-flex items-center rounded bg-green-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-green-700">Lihat</a>

                                    @if(auth()->user()->role->nama_role === 'staff')
                                        <a href="{{ route('pembudidaya.edit', $p->id_pembudidaya) }}" class="inline-flex items-center rounded bg-yellow-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-yellow-600">Edit</a>
                                    @endif

                                    @if(auth()->user()->isAdmin() && $p->status === 'pending')
                                        <form action="{{ route('pembudidaya.verify', $p->id_pembudidaya) }}" method="POST" class="inline">
                                            @csrf
                                        ...
                                        <form action="{{ route('pembudidaya.reject', $p->id_pembudidaya) }}" method="POST" class="inline form-reject-catatan" data-entity="data pembudidaya ini">
                                            @csrf
                        ...
                        @empty
                            <div class="rounded-lg border border-slate-200 p-4 text-center text-slate-500">Belum ada data pembudidaya.</div>
                        @endforelse
                    </div>
                    ...
                    <div class="rounded-md border border-slate-300 overflow-hidden">
                        <table class="min-w-full text-base">
                            ...
                            <tbody>
                                ...
                                @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-6 text-center text-slate-500">Belum ada data pembudidaya.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
